 ...
 Page({
   ...
   onLoad: function(options) {
    ...
    let goodsType = options.type
    // 获取商品按份具体费用
    let changeObj = this.filterLocalList(goodsType)
    options.orderList = JSON.stringify(changeObj.localList)
    options.cart = JSON.stringify(changeObj.cart)
   ...
   initData (options) {
    ...
    let feeConfig = JSON.parse(options.feeConfig)

    let dtTotalFee = 0; // 按份和不按份抽成费用
    let rewardTotalFee = 0; // 按份不按份骑手总费用
    // 不按份收取的总价
    let lyFee = 0
    ...
    orderList.forEach((item)=>{
      if (!item.settlement) {
        ...
        dtTotalFee = np.plus(dtTotalFee, np.times(item.dtFee, item.num))
        rewardTotalFee = np.plus(rewardTotalFee, np.times(item.rewardFee, item.num))
      }
    })

    // 获取不按份附加费用    
    let extInfo = this.getExtracharge(feeConfig, lyFee, options.type);
    console.log('附加配置命中：', extInfo)
    let surcharge = extInfo.setExtracharge;
    // 附加费总价
    let surchargeFee = extInfo.extrachargeFee;
    ...
    let rewardInfo = this.getRewardInfo(feeConfig, lyFee, options.type)

    let rewardFee = rewardInfo.rewardFee
    console.log('配送费命中', rewardFee)
    rewardTotalFee = np.plus(rewardTotalFee, rewardFee)

    // 获取抽成费用
    let dtInfo = this.getDtFee(feeConfig, lyFee, options.type)
    
    console.log('抽成费用-------------------', dtInfo)
    // 不按分抽成
    dtTotalFee = np.plus(dtTotalFee, dtInfo.fee)

    
    // 配送费和抽成费总费用
    let totalDivide = np.plus(dtTotalFee, rewardTotalFee)
    ...
    let settlementTotal = np.minus(feeTotal, totalDivide)

    let settlementTotalSc = np.times(settlementTotal, 0.006)

    settlementTotal = np.minus(settlementTotal, settlementTotalSc)

    dtTotalFee = Math.floor(dtTotalFee * 100)/100
   ...
   // 保留两位小数
    ...
    console.log(`
      ...
      配送支出: ${rewardTotalFee} 
      平台抽成：${dtTotalFee}
      提现手续费：${settlementTotalSc}
      最终提现费用: ${settlementTotal}
    `)
    this.setData({
      orderList: orderList,
      cart: cart, // 除了附加费的费用
      ...
      surcharge: surcharge, // 附加费 [{label: '配送费'， price: '0.1'}]
      feeTotal: feeTotal, // 订单总价
      settlement: {
       // 总收入 
       total: feeTotal,
       // 抽成总费用
   ...
   // 获取抽成费用
   getDtFee (feeConfig, currentFee, type) {
   ...
   // 按规则获取配送费
   getRewardInfo (feeConfig, fee, type) {
   ...
   // 附加费配置  当前费用  当前类型
   getExtracharge (feeConfig, fee, type) {
     // sultType: [] 适用类型
     ...
     if (extracharge) {
       ...
       extracharge.forEach((item) => {
        if (item.trigger === 'bycost' && item.sultType.indexOf(type) > -1) {
         if (item.trigger === 'bycost') {
           item.rule.some((ruleItem) => {
              ...
              if (section1 > 0) {
                if (currentFee > 0 && currentFee >= section0 && currentFee < section1) {
                  setExtracharge.push({
                    label: item.label,
                    price: ruleFee
                  })
   ...
   filterLocalList (type) {
    ...
    _localList.forEach((item) => {
      ...
      if (item.surcharge && item.surcharge.length > 0) {
       ...
       // 按份附加费处理
       let getExtrachargeInfo = that.FengetExtracharge(item.price, type)
       item.surcharge = getExtrachargeInfo.setExtracharge
       item.extrachargeFee = getExtrachargeInfo.extrachargeFee
       // 按份配送费
       let getRewardInfo = that.FengetRewardInfo(item.price, type)
       item.rewardFee = getRewardInfo.rewardFee
       // 按份抽成费用
       let getDtFeeInfo = that.FengetDtFee(item.price, type)
       item.dtFee = getDtFeeInfo.fee
      }
  ...
  // 按份设置
  // 按份附加费
  FengetExtracharge (fee, type) {
    // sultType: [] 适用类型
    let feeConfig = JSON.parse(this.data.cloneOptions.severalConfig)
    let extracharge = feeConfig.extracharge
    // 用于展示
    if (extracharge) {
      let currentFee = Number(fee)
    ...
    if (dfConfig) {
      ...
      if (currentType === 'fixed') {
        ...
        currentValue.some((item) => {
          ...
          let section1 = Number(section[1])
          if (fee >= section0 && fee <section1) {
            console.log('走进来了------------------', fee, item, type)
            if (item.type === 'bytype') {
              returnFee = Number(item.typefee[type])
              console.log(returnFee, 'oooooooooooooooooooooooo')
            } else {
              returnFee = Number(item.fee)
            }
            isState = true
